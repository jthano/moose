                 ...
                 const std::set<std::string> & ic_deps)
{
  ...
  for (const ExecFlagType & flag : execute_flags.items())
  {
    ...
    for (const auto obj : objs)
    {
      if (depend_objects_aux.count(obj->name()) > 0 && flag != EXEC_INITIAL)
        pre_aux_dependencies[obj].insert(flag);
      else if (depend_objects_aux.count(obj->name()) > 0 && flag == EXEC_INITIAL)
        is_pre_ic[obj] = true;
      else if (flag != EXEC_INITIAL)
        // default is for UO to be post aux. If EXEC_INITIAL, check first if UO
        // will be dependent on IC or have force_preic before decided to put in
        // post_aux
  ...
  for (const auto obj : objs)
  {
    ...
    if ((obj->isParamValid("force_preaux") && obj->template getParam<bool>("force_preaux")))
    {
      ...
      for (const ExecFlagType & flag : execute_flags.items())
        if (flag != EXEC_INITIAL || !is_pre_ic[obj])
          pre_aux_dependencies[obj].insert(flag);
    }
    else
    {
